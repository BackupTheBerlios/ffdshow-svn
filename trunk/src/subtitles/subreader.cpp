...
subtitle* sub_read_file (const char *filename, float fps) {
    ...
    while(1){
        ...
        if(sub_num>=n_max){
            n_max+=16;
            first=(subtitle*)realloc(first,n_max*sizeof(subtitle));
        }
	sub = &first[sub_num];
...
char * sub_filename(const char* path, const char * fname )
{
 ...
 sub_name1=(char*)malloc(strlen(fname)+8);
 strcpy(sub_name1,fname);

 sub_name2=(char*)malloc (strlen(path) + strlen(fname) + 8);
